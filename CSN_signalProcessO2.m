...
plot(Tnew{3},INA{3});
figure(3);clf;
plot(T1-18.8,CSNA1,'k');hold on;
plot(Tnew{1}-18.8,PS{1},'r');hold on;
plot(Tnew{1}-18.8,NS{1},'r');hold on;
xlim([0 0.2]);
ylim([-65 65])
pbaspect([6,1,1]);
figure(4);clf;
...
xlabel('MAP (mmHg)')
ylabel('voltage (mV)')
save baseline.mat nM fitresult1 nina
